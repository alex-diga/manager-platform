...
for(var j=0;j<4;j++){if(i*8+j*6>binarray.length*32){str+=b64pad}else{str+=tab.charAt((triplet>>6*(3-j))&63)}}}return str};
	...
	var CONTENT_TYPE_MULTIPART = 'multipart/form-data';
	var CONTENT_TYPE_JSON = 'application/json';

	var XMLHTTPREQUEST_KEY = 'X-Requested-With';
	var XMLHTTPREQUEST_VALUE = 'XMLHttpRequest';
	...
	var ajax = {
		...
		request:function(opts) {
			...
			var error = opts.error || function(e){
	...
	function request(opts) {
		...
		// var jwt = localStorage.getItem('easyconf_jwt')
	    if(jwt) {
